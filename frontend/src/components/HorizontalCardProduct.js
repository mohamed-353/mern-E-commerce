          ...
          {loading ? (
            ...
            products.map((product) => (
              <Link
                ...
                className='flex w-full h-44 min-w-96 max-w-96 rounded-lg bg-white shadow-md overflow-hidden'
              >
                <div className='bg-slate-200 h-full w-44 p-3 overflow-hidden cursor-pointer flex justify-center items-center'>
                  <img src={product?.productImage[0]} alt={product?.productName} className='h-full object-scale-down hover:scale-110 transition-all duration-300 mix-blend-multiply' />
                </div>

                <div className='p-4 grid'>
                  <div>
                    <h3 className='text-lg text-ellipsis line-clamp-1 font-semibold truncate'>{product?.productName}</h3>
                    <p className='text-lg text-slate-500 capitalize truncate'>{product?.category}</p>
                  </div>
                  ...
                  <button
                    className='bg-cyan-600 hover:bg-cyan-700 transition-all duration-300 text-white flex items-center justify-center px-4 rounded-full'
